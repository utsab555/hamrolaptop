...
if ($laptopId) {
    $stmt = $conn->prepare("SELECT * FROM laptops WHERE l_id=? and category='budget'");
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($l_image) {
        ...
        $stmt = $conn->prepare("SELECT l_image FROM laptops WHERE l_id=? and category='budget'");
        $stmt->bind_param("i", $laptopId);
        $stmt->execute();
    ...
    if ($laptopId) {
        $stmt = $conn->prepare("UPDATE `laptops` SET l_name=?, l_model=?, l_processor=?,l_ram=?,l_storage=?,l_display=?, l_amount=?,l_addinfo=?, l_image=? WHERE l_id=? and category='budget'");
